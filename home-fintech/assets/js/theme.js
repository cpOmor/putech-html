    ...
    if ($('.testimonial-slider').length) {
        $('.testimonial-slider').slick({
            ...
            customPaging: function (slider, i) {
                return (
                    '<button class="w-4 h-4 border-2 border-black rounded-full flex items-center justify-center transition-all duration-300">' +
                    '<span class="w-2 h-2 bg-transparent rounded-full transition-all duration-300"></span>' +
                    '</button>'
                );
            },
            responsive: [
